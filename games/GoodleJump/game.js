...
let gravity = 12.5; // 16 for IPhone
let jumpPower = 28; // 22 for IPhone
let isJumping = false;
...
function updatePlatforms() {
  if (player.y < canvas.height / 1.15) {
    platforms.forEach(p => p.y += 6); // 7 for IPhone
  }
...
function handleTouchStart(event) {
  ...
  player.dx = touchX < canvas.width / 2 ? -10 : 10; // -17:17 for IPhone
}
